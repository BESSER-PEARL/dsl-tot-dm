...
class GeneratorPrompt():
    ...
    def prompt(self, nfocus):
        prompt = f'You are a domain modeling expert that creates a domain model with a given purpose:\n{{purpose}}\n'
        if nfocus > 1:
            prompt +=f"The focus of the model is: {{model_focus}}.\n"
        if nfocus > 0:
            prompt +=f"Create the model from the perspective of focal actors: {{focal_actors}}.\n\n"
...
class EvaluatorPrompt():
    ...
    def prompt(self, nfocus, nCriterias):
        prompt = 'You are a domain modeling expert that decides which choice is the best model.\n'
        prompt +=f'The domain model has the following purpose:\n{{purpose}}\n'
        if nfocus > 1:
            prompt +=f"The focus of the model is: {{model_focus}}.\n"
        if nfocus > 0:
            prompt +=f"Create the model from the perspective of focal actors: {{focal_actors}}.\n\n"
