...
class Level():
    ...
    def setup_prompts(self, modeling_problem: ModelingProblem):
        ...
        self.generator_prompt.prompt(1)
        self.evaluator_prompt.prompt(1,0)

    def generate_prompt(self, prompt_name, thought = ''):
        inputs_values = self.modeling_problem.focus_actors
        model_focus = inputs_values[1] if len(inputs_values) > 1 else ""
        focal_actors = inputs_values[0] if len(inputs_values) > 0 else ""
        prompt = ''
        if prompt_name == 'generator':
            prompt = self.generator_prompt.prompt(len(self.modeling_problem.focus_actors))
            prompt = prompt.format(
                purpose=self.modeling_problem.get_purpose(), 
                model_focus=model_focus,
                focal_actors=focal_actors,
                domain=self.modeling_problem.get_domain_description(),
                tasks=self.modeling_problem.get_tasks(),
                thought = thought)
        if prompt_name == 'evaluator':
            prompt = self.evaluator_prompt.prompt(len(self.modeling_problem.focus_actors), self.modeling_problem.get_assessment()[self.id-1]["nCriterias"])
            prompt = prompt.format(
                purpose=self.modeling_problem.get_purpose(), 
                model_focus=model_focus,
                focal_actors=focal_actors,
