...
def run():
    ...
    modelingTasks = [''] * (4)
    modelingTasks[0] = ModelingTask(level = 1, name = "Extract Atomic Legal Statements from Law Article")
    modelingTasks[0].set_description("""Your task is to extract atomic legal statements from the law article given, focusing on the specified actors. Please note that atomic legal statements have the format: one <actor> (the subject), one <modal verb> (modality), one to many <actions>, and 0 to many <precondition> under which the action(s) should be taken. 
...
Take into account that an atomic statement can have no precondition.""")
    modelingTasks[0].add_assessment("""Criterion 1: make sure atomic statements are extracted from the law article from the perspective of the focal actor(s).""")
    modelingTasks[0].add_assessment("""Criterion 2: make sure that preconditions are extracted from the law article for clauses start with if.""")
    problem.add_task(task = modelingTasks[0])
    modelingTasks[1] = ModelingTask(level = 2, name = "Classify and Format Atomic Legal Statements")
    modelingTasks[1].set_description("""Your task is to: (1) classify each atomic legal statement extracted in the previous step to either an obligation or a permission, based on the modal verb of the legal statement; (2) format each classified atomic legal statement with the template provided below:
...
Do not include the Precondition(s) under which the Action(s) should be taken in the Action(s).""")
    modelingTasks[1].add_assessment("""Criterion 1: make sure description of action(s) do not include description of precondition(s).""")
    modelingTasks[1].add_assessment("""Criterion 2: every atomic legal statement should be classified as either an obligation or a permission.""")
    problem.add_task(task = modelingTasks[1])
    modelingTasks[2] = ModelingTask(level = 3, name = "Model Atomic Legal Statements in Legal GRL")
    modelingTasks[2].set_description("""Your task is create one Legal GRL model capturing the classified and formatted atomic legal statements from the previous step. Legal GRL is a DSML for modeling regulatory knowledge. Here is the Legal GRL syntax in TextX format:
...
4. Connect the «Precondition» created in step 3 to the «Goal» created in step 2  with a Dependency, whereby the «Precondition» is the source.""")
    modelingTasks[2].add_assessment("""Criterion 1: a «Precondition», namely an IntentionalElement of type Goal with modality Precondition, must be the source of a Dependency.""")
    modelingTasks[2].add_assessment("""Criterion 2: the source of a Dependency must be a «Precondition», and the target of a Dependency must be either an «Obligation» or an «Permission».""")
    modelingTasks[2].add_assessment("""Criterion 3: no duplication of model elements with same description.""")
    problem.add_task(task = modelingTasks[2])
    modelingTasks[3] = ModelingTask(level = 4, name = "Refine Legal GRL model")
    modelingTasks[3].set_description("""Your task is to refine «Obligations», «Permissions» and «Preconditions» in the Legal GRL model created in the previous step. Your output, namely the refined Legal GRL model, should be formatted with the Legal GRL syntax defined by a TextX grammar.
...
(5) Create contribution links to defines the level of impact that the satisfaction of a source IntentionalElement has on the satisfaction of a destination IntentionalElement. The level of impact is between [-100, 100], with 100 meaning the satisfaction of the source IntentionalElement sufficiently makes the satisfaction of the destination IntentionalElement, and -100 meaning the satisfaction of the source IntentionalElement sufficiently breaks the satisfaction of the destination IntentionalElement. Draw contribution links among goals or soft goals when pertinent.""")
    modelingTasks[3].add_assessment("""Criterion 1: Decomposition is only between IntentionalElements of the same type and modality, namely «Precondition» are only decomposed into «Preconditions», «Obligation» are only decomposed into «Obligations», and «Permission» are only decomposed into «Permissions».""")
    modelingTasks[3].add_assessment("""Criterion 2: The target of the Decomposition must be more than one Intentional Element.""")
    problem.add_task(task = modelingTasks[3])


    tree.setup_tree()
    
    logName = "clerk_legalgrl.log"
    output = tree.execute(logName)

    output_filename = "output_legalgrl.txt"
